...
const Card = ({title,landMark,image1,image2,image3,rent,city,id,ratings,isBooked,host}) => {
  ...
  return (
       ...
       {isBooked &&<div className='text-[green] bg-white rounded-lg absolute flex items-center justify-center right-1 top-1 gap-[5px] p-[5px] text-xs'><GiConfirmed className='w-[20px] h-[20px] text-[green]'/>Booked</div>}

       {isBooked && host == userData?._id && <div className='text-[red] bg-white rounded-lg absolute flex items-center justify-center right-1 top-[50px] gap-[5px] p-[5px] text-xs'  onClick={() =>setPopUp(true)}><FcCancel  lassName='w-[20px] h-[20px]'
       />Cancel Booking</div>}
       ...
       {popUp && <div className='w-[300px] h-[100px] bg-[#ffffffdf] absolute top-[110px] left-[13px] rounded-lg'>
        ...
        <div className='w-[100%] h-[50%] text-[#986b6b] font-semibold flex items-start justify-center gap-[10px] text-[18px]'>
          Are you sure? <button className='px-[20px] bg-[red] text-[white] rounded-lg hover:bg-slate-600' onClick={()=>{cancelBooking(id);setPopUp(false)}}>Yes</button> <button className='px-[20px] bg-[red] text-[white] rounded-lg hover:bg-slate-600'  onClick={() =>setPopUp(false)}>No</button>
        </div>
       </div>}
      ...
      <div className='w-[100%] h-[33%] py-[20px] flex flex-col gap-[2px]'>
          ...
          <span className='flex items-center justify-center gap-[5px]'><FaStar className='text-[#f4f48d]'/>{ratings}</span>
          </div>
